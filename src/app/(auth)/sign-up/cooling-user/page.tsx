...
import { signUpCoolingUserSchema } from "@/constants/schemas";
...
export default function SignUpCoolingUserPage() {
  ...
  const onSubmit = async (values: any) => {
    try {
      ...
      const res = await authService.signUpAsCoolingUser({ ...values, phone: normalizedPhone });
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 p-8">
      ...
      <div className="flex items-center gap-3 mb-7">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">Create Farmer Account</h1>
          <p className="text-xs text-gray-500">Store your produce and access cold storage</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        {/* Cooling Unit ID */}
        <div>
          <label htmlFor="coolingUnitId" className="block text-sm font-medium text-gray-700 mb-1.5">
            Cooling Unit ID
          ...
          <input
            ...
            onChange={(e) => setValue("coolingUnitId", e.target.value ? Number(e.target.value) : undefined)}
