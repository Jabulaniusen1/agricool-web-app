...
function AvatarImage({ className, src, alt = "", onLoad, onError, ...props }: React.ComponentProps<"img">) {
  ...
  return (
    <img
      src={src}
      alt={alt}
      onLoad={(e) => { setImgLoaded(true); onLoad?.(e) }}
...
function AvatarFallback({ className, ...props }: React.ComponentProps<"span">) {
  ...
  return (
    <span
      className={cn("flex size-full items-center justify-center rounded-full bg-gray-200 text-sm font-medium text-gray-600", className)}
      {...props}
    />
  )
}
...
function AvatarBadge({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      className={cn("absolute right-0 bottom-0 z-10 inline-flex items-center justify-center rounded-full bg-gray-900 text-white ring-2 ring-white size-2.5", className)}
      {...props}
    />
  )
}
...
function AvatarGroupCount({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn("relative flex size-8 shrink-0 items-center justify-center rounded-full bg-gray-200 text-sm text-gray-600 ring-2 ring-white", className)}
      {...props}
    />
  )
}
